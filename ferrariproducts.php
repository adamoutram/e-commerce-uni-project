...
<html lang="en">
	...
	<body>
		...
		<div id="content">
			...
			<br />			
			<h2 align="center">Ferrari Products</h2>
			<form name="sort" action="ferrariproducts.php" method="post">
	 ...
	 $query = "SELECT * FROM products WHERE Brand = 'Ferrari' ORDER BY ".$sort." DESC" or die(mysqli_error());

} else { // else if you do not pass any value from select option will return this
	    $query = "SELECT * FROM products WHERE Brand = 'Ferrari'" or die(mysqli_error());
}
...
<th>Specification</th>
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			<br />
			" . '
			<form id="form1" name="form1" method ="post" action="cart.php"> <input type="hidden"
			name="pid" id="pid" value="'.$row["Product_ID"].'"/>
			<input type="submit" name ="button" id="button" value="Add to Cart" /></form> '."</td>"; 
